...
{
// IN CASE IT WAS TURNED OFF TO SAVE POWER	
...
//            SD16SSEL_1;  // SMCLK clock sel for ADC  
  ...
  SD16INCTL0 = inctl0;  // input selection -- gain == 1 , 4th conversion causes interrupt
   ...
   SD16CCTL0 = SD16OSR_1024|   // oversampling rate 512
...
// ADC setup      
    SD16CTL = SD16REFON|    // Internal 1.2V Ref ON
              SD16SSEL_1;   // SMCLK clock sel for ADC  
    SD16AE = 0;     		// no differential analog imputs , single ended use only  
...
extern unsigned short etext; 		// linker defined symbol at the end of the text segment
...
void main( void )
{
  ...
  flash = &etext; // get the beginning of unused flash, i.e. the end of the code (text) segment
...
// TODO -- This can only be written once -- Flash takes damadge when location is witten multiple times without an erase in between 
  *flash++ = 0xcafe;
